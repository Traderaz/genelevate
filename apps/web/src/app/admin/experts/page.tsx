...
import { Plus, Edit2, Trash2, Save, X, Upload, Linkedin, Twitter, Mail, Loader2, Award } from 'lucide-react';
...
export default function AdminExpertsPage() {
  ...
  const fetchExperts = async () => {
    try {
      ...
      const q = query(expertsRef, orderBy('order', 'asc'));
  ...
  const handleUpdateExpert = async (id: string) => {
    try {
  ...
  const startEdit = (expert: Expert) => {
    setFormData({
      ...
      socialLinks: {
        ...
        email: expert.socialLinks?.email || ''
      },
      expertise: expert.expertise && expert.expertise.length > 0 ? expert.expertise : [''],
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filename = `experts/${timestamp}_${sanitizedName}`;
      ...
      <div className="min-h-screen bg-background p-8">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Manage Experts</h1>
              <p className="text-muted-foreground mt-2">
                Add, edit, or remove experts from the homepage
              </p>
            </div>
            <button
              onClick={() => {
          ...
          {(showAddForm || editingId) && (
            <div className="bg-card border border-border rounded-xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 bg-background border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                    placeholder="Mathematics Expert & Oxford Graduate"
                  />
                </div>
                    ...
                    {formData.expertise.map((item, index) => (
                      <div key={index} className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2 bg-background border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                          placeholder="e.g., Advanced Mathematics, Calculus, Problem-Solving Strategies"
                        />
                        <button
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  onClick={() => editingId ? handleUpdateExpert(editingId) : handleAddExpert()}
                  className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors flex items-center gap-2"
                >
                  <Save className="w-5 h-5" />
                  {editingId ? 'Update' : 'Add'} Expert
                </button>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded text-xs ${expert.isActive ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10 text-gray-500'}`}>
            ...
            {experts.length === 0 && !showAddForm && (
              <div className="text-center py-12 text-muted-foreground">
                <p>No experts added yet. Click "Add Expert" to get started.</p>
              </div>
            )}
          </div>
        </div>

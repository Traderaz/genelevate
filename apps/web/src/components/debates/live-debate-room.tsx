...
import { DailyProvider, useDaily, useDailyEvent, useParticipantIds, useLocalParticipant, useScreenShare } from '@daily-co/daily-react';
import { useEffect, useRef, useState } from 'react';
import { X, Mic, MicOff, Video as VideoIcon, VideoOff, Monitor, MonitorOff, MessageSquare, Users as UsersIcon, LogOut, Hand, Loader2 } from 'lucide-react';
...
function DebateCall({ roomUrl, isParticipant, userName, onLeave }: LiveDebateRoomProps) {
  ...
  const sendChatMessage = () => {
    if (chatInput.trim() && daily && hasJoined) {
      try {
        ...
        toast.error('Please wait until you have fully joined the room');
      }
    }
  };
  ...
  useDailyEvent('participant-joined', (ev) => {
    if (ev?.participant) {
      toast.success(`${ev.participant.user_name || 'Someone'} joined the debate`);
    }
  });

  useDailyEvent('participant-left', (ev) => {
    if (ev?.participant) {
      toast.info(`${ev.participant.user_name || 'Someone'} left the debate`);
    }
  });
  ...
  useEffect(() => {
    if (daily) {
      console.log('🎬 Daily instance ready, attempting to join...');
      ...
      daily.join({
        ...
        // If not a participant (viewer), turn off camera and mic
        if (!isParticipant) {
        ...
        toast.error('Failed to join debate room');
      });
    }
  }, [daily, userName, isParticipant, roomUrl]);
    ...
    <div className="flex h-screen bg-gradient-to-br from-teal-primary to-teal-gold text-white">
      ...
      <div className="flex-1 flex flex-col bg-gray-900 relative">
        ...
        <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/80 to-transparent p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-semibold">{participantCount} in room</span>
            </div>
          </div>
          <button
            onClick={onLeave}
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-20 pt-24 overflow-y-auto">
          {/* Local Participant (if participant) */}
          {localParticipant && isParticipant && (
            <div className="relative bg-black rounded-xl overflow-hidden border-2 border-teal-gold shadow-2xl aspect-video">
              <video
                autoPlay
                muted
                playsInline
              ...
              <div className="absolute bottom-4 left-4 bg-black/70 backdrop-blur-sm px-3 py-1 rounded-lg">
                <span className="font-semibold text-white">{userName} (You)</span>
              </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-6 flex justify-center gap-4">
          <Button
            ...
            title={isParticipant ? "Toggle Microphone" : "Viewers cannot unmute"}
          >
          ...
          <Button
            ...
            title={isParticipant ? "Toggle Camera" : "Viewers cannot enable camera"}
          >
            {localParticipant?.videoTrack ? <VideoIcon className="w-6 h-6" /> : <VideoOff className="w-6 h-6" />}
          </Button>
      ...
      {showParticipants && (
        <div className="w-80 bg-gray-800 flex flex-col border-l border-white/10">
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <UsersIcon className="w-5 h-5 text-teal-gold" />
...
export function LiveDebateRoom({ roomUrl, isParticipant, userName, onLeave }: LiveDebateRoomProps) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-teal-primary to-teal-gold text-white z-50">
        ...
        <p className="text-xl font-semibold">Loading debate room...</p>
  ...
  if (error) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-red-800 to-red-600 text-white z-50 p-4 text-center">
        ...
        <p className="text-2xl font-bold mb-2">Error Joining Debate</p>
  ...
  return (
    <DailyProvider url={roomUrl}>
      <DebateCall roomUrl={roomUrl} isParticipant={isParticipant} userName={userName} onLeave={onLeave} />
    </DailyProvider>
  );
}

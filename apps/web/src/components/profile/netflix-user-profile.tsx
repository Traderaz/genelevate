    ...
    <>
        ...
        <div className="relative z-10 p-8">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <div className="relative group">
              <div className="w-32 h-32 bg-gradient-to-br from-teal-primary to-teal-gold rounded-full flex items-center justify-center text-4xl font-bold text-white overflow-hidden shadow-lg">
              ...
              <div className="flex flex-wrap gap-2">
                {userProfile.subjects && userProfile.subjects.length > 0 ? (
                  userProfile.subjects.map((subject) => (
                    <span
                      key={subject}
                      className="px-3 py-1 bg-teal-gold/20 text-teal-gold border border-teal-gold/30 rounded-full text-sm font-medium"
                    >
                      {subject}
                    </span>
                  ))
                ) : (
                  <span className="text-sm text-white/60 italic">No subjects added yet</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <div
            key={stat.label}
            className="teal-card border-2 border-teal-blue-medium/30 rounded-xl p-6 netflix-card shadow-lg"
          >
            ...
            <div className="space-y-1">
              <p className="text-2xl font-bold text-[#0B5C9E]">{stat.value}</p>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-6 border-b-2 border-white/20 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {activeTab === 'profile' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="teal-card border-2 border-teal-blue-medium/30 rounded-xl p-6 shadow-lg">
                <h3 className="text-lg font-semibold text-[#0B5C9E] mb-4">Personal Information</h3>
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <User className="w-5 h-5 text-teal-primary" />
                    <div>
                      <p className="text-sm text-[#666666] font-medium">Full Name</p>
                      <p className="font-semibold text-[#0B5C9E]">
                        {userProfile.displayName || `${userProfile.firstName} ${userProfile.lastName}`}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-[#666666] font-medium">Email</p>
                      <p className="font-semibold text-[#0B5C9E]">{userProfile.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Calendar className="w-5 h-5 text-teal-primary" />
                    <div>
                      <p className="text-sm text-[#666666] font-medium">Member Since</p>
                      <p className="font-semibold text-[#0B5C9E]">
              ...
              <div className="teal-card border-2 border-teal-blue-medium/30 rounded-xl p-6 shadow-lg">
                <h3 className="text-lg font-semibold text-[#0B5C9E] mb-4">Academic Information</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-[#666666] font-medium">Year Group</p>
                    <p className="font-semibold text-[#0B5C9E]">
                      {yearGroupInfo ? `${yearGroupInfo.emoji} ${yearGroupInfo.displayName}` : 'Not set'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-[#666666] font-medium">Subjects</p>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {userProfile.subjects && userProfile.subjects.length > 0 ? (
                        userProfile.subjects.map((subject) => (
                          <span
                            key={subject}
                            className="px-2 py-1 bg-teal-primary/20 text-teal-primary border border-teal-primary/40 rounded text-sm font-semibold"
                          >
                            {subject}
                          </span>
                        ))
                      ) : (
                        <span className="text-sm text-[#999999] italic">No subjects added</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <div className="teal-card border-2 border-teal-blue-medium/30 rounded-xl p-6 shadow-lg">
                <h3 className="text-lg font-semibold text-[#0B5C9E] mb-4">Subscription</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-[#0B5C9E] capitalize">{userProfile.subscription.plan} Plan</p>
                  ...
                  {userProfile.subscription.plan === 'free' ? (
                    <button 
                      ...
                      className="w-full px-4 py-3 bg-gradient-to-r from-teal-primary to-teal-blue-medium text-white font-semibold rounded-lg hover:opacity-90 transition-opacity shadow-lg"
                    >
                      Upgrade to Premium
                    </button>
                  ) : (
                    <button 
                      onClick={() => setShowSubscriptionManager(true)}
                      className="w-full px-4 py-3 teal-button-primary"
                    >
              ...
              <div className="teal-card border-2 border-teal-blue-medium/30 rounded-xl p-6 shadow-lg">
                <h3 className="text-lg font-semibold text-[#0B5C9E] mb-4">Quick Actions</h3>
                <div className="space-y-3">
                  <button className="w-full flex items-center gap-3 px-4 py-3 bg-teal-gold/20 hover:bg-teal-gold/30 border border-teal-gold/40 rounded-lg transition-colors">
                    <Shield className="w-5 h-5 text-teal-primary" />
                    <span className="text-[#0B5C9E] font-semibold">Privacy Settings</span>
                  </button>
                  <button className="w-full flex items-center gap-3 px-4 py-3 bg-teal-gold/20 hover:bg-teal-gold/30 border border-teal-gold/40 rounded-lg transition-colors">
                    <Bell className="w-5 h-5 text-teal-primary" />
                    <span className="text-[#0B5C9E] font-semibold">Notification Preferences</span>
                  </button>
                  <button className="w-full flex items-center gap-3 px-4 py-3 bg-teal-gold/20 hover:bg-teal-gold/30 border border-teal-gold/40 rounded-lg transition-colors">
                    <CreditCard className="w-5 h-5 text-teal-primary" />
                    <span className="text-[#0B5C9E] font-semibold">Billing Information</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {achievements.map((achievement, index) => (
                <div
                  ...
                  className={`p-6 rounded-xl border-2 text-center netflix-card shadow-lg ${
                    achievement.unlocked
                      ? 'teal-card border-teal-blue-medium/30'
                      : 'bg-gray-100/50 border-gray-300 opacity-60'
                  }`}
                >
                  <div className="text-4xl mb-4">{achievement.icon}</div>
                  <h4 className="font-semibold text-[#0B5C9E] mb-2">{achievement.name}</h4>
                  <p className="text-sm text-[#666666] font-medium">{achievement.date}</p>
                  {achievement.unlocked && (
                    <div className="mt-3 px-3 py-1 bg-green-500/20 text-green-600 border border-green-500/40 rounded-full text-xs font-bold inline-block">
                      Unlocked
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {recentActivity.map((activity, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 p-4 teal-card border-2 border-teal-blue-medium/30 rounded-xl shadow-lg"
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    ...
                    {activity.type === 'webinar' && <Eye className="w-5 h-5 text-red-600" />}
                    {activity.type === 'achievement' && <Star className="w-5 h-5 text-yellow-600" />}
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-[#0B5C9E]">
                      {activity.action} {activity.item}
                    </p>
                    <p className="text-sm text-[#666666] font-medium">{activity.time}</p>
                  </div>
                </div>
              ))}
            </div>
